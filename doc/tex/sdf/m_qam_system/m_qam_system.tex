...
\begin{figure}[H]
	...
	\caption{Probability density functions for $s(t) = A(t) + n(t)$, with $m(t)=\pm A$ and $n(t)$ as a Gaussian variable.}
	\label{fig:gauss}
\end{figure}
...
&N &= n_0 B~\text{? Ver isto que não deve estar correcto}
\end{eqnarray}
%\begin{equation}\label{eq:berBPSK}
%BER= Q\left({G_{ele}\sqrt{\frac{2 P_L P_S}{n_0}}}\right) = \frac{1}{2} \text{erfc}\left({G_{ele}\sqrt{\frac{P_L P_S}{n_0}}}\right).
%\end{equation}
...
The peak SNR, defined as the ration between the peak signal power at the sampling instant to mean noise power, is given by
...
\noindent where $T_s$ is the symbol period, $B$ is the bandwidth of the system, $A$ is as defined in Equation~\ref{eq:bpskamplitude}, the average peak amplitude of the signal without noise prior to filtering.
...
\begin{minipage}[b]{0.5\linewidth}
	\begin{table}[H]
		...
		\begin{tabulary}{1.0\textwidth}{|p{3.5cm}|p{3.5cm}|}
			\hline
			\multicolumn{2}{|l|}{ \textbf{Simulation and Curve Parameters} } \\
			\hline
			\textbf{Parameter}     & \textbf{Default Value}                                     \\\hline
			numberOfBitsGenerated  & $40000$	                                                \\\hline
			samplingRate           & 															\\\hline
