...
\hspace*{1cm}  University of Aveiro\\ %Instituto\\ Superior T�cnico, Instituto de Telecomunica��es\\
%
...
Uses time bin properties and the main principle is the \textbf{Quantum Entanglement}.

It is also characterized by having a very simple experimental setup since Bob's apparatus e passive.

    \begin{figure}[hbt]
    	\centering
    	\includegraphics[width=0.6\textwidth, height=6cm]{./figures/Alice.pdf}
        	\label{bob}
    \end{figure}
...
All the other photons should click the $D_{M1}$.
  \begin{figure}[hbt]
    	\centering
    	\includegraphics[width=0.45\textwidth]{./figures/Simple2.png}
        	\label{Simple2}
    \end{figure}
%--------------------------------------------------------------------------------------------------
%------------ SLIDE -------------------------------------------------------------------------------
...
\item [Step 3] Bob informs Alice when $D_{M1}$ and $D_{M2}$ clicked. Alice tell Bob which items of the data line must be discarded because they correspond to decoy sequences.

\item [Step 4] They calculate the visibility (V) and the QBER (Q) of the key.
$$V_s=\frac{P(D_{M1}|s)-P(D_{M2}|s)}{P(D_{M1}|s)+P(D_{M2}|s)}$$

where $P(D_{M}|s)$ is the probability that detector $D_{M}$ has clicked at a time corresponding to a s sequence. After that Bob reveals some bits of the data line to calculate the Q.
\end{description}
...
\vspace*{5.5cm}
\begin{itemize}
	\item 2014 - Singh et al. - Quantum Key Distribuition Protocols A review
	\item 2006 - Branciard et al. - Zero-Error Attacks and Detection Statistics in the Coherent One-Way Protocol for Quantum Cryptography
	\item 2017 - Kronberg et al. - Analysis of coherent quantum cryptography protocol
	\item 2017 - Roberts et al. - Modulator-free coherent-one-way quantum key distribution
\end{itemize}
