...
bool SuperBlock::runBlock() {
	...
	do {
		...
		for (unsigned int i = 0; i < outputSignals.size(); i++) {
			...
			switch (sType) {
			case signal_value_type::BinaryValue:
				...
				break;
			case signal_value_type::RealValue:
				for (int j = 0; j < length; j++) {
					t_real signalValue;
					moduleBlocks[moduleBlocks.size() - 1]->outputSignals[i]->bufferGet(&signalValue);
					outputSignals[i]->bufferPut(signalValue);
				...
				break;
			case signal_value_type::ComplexValue:
				for (int j = 0; j < length; j++) {
				...
				break;
			case signal_value_type::ComplexValueXY:
				for (int j = 0; j < length; j++) {
...
RealToComplex::RealToComplex(vector <Signal *> &InputSig, vector <Signal *> &OutputSig) {
	...
	setNumberOfInputSignals(InputSig.size());
	setNumberOfOutputSignals(OutputSig.size());
