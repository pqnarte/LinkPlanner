...
\begin{table}[H]
    ...
    \end{tabulary}
    \caption{$^1$ The help entry is under a different name, \textit{m\_qam\_receiver};\\$^2$ Has been changed to remove pulse shaper for now.\label{tab:sources}}
\end{table}
...
\begin{table}[H]
    ...
    \end{tabulary}
    \caption{$^1$ The help entry is under a different name, \textit{m\_qam\_receiver};\\$^2$ Has been changed to remove pulse shaper for now;\\$^3$ This file does not share the same time-tag as the corresponding ".cpp" file.\label{tab:headers}}
\end{table}
