	...
	for (long int i = 0; i < process; i++) {
		...
		if (m > 0)
		{
			if ((remainder(receivedBits_mid, m) == 0) & (receivedBits_mid > 0))
			{
				...
				double BER;
				BER = (receivedBits_mid - coincidences_mid) / receivedBits_mid;

				double UpperBound = BER + 1 / sqrt(receivedBits_mid) * z  * sqrt(BER*(1 - BER)) + 1 / (3 * receivedBits_mid)*(2 * z * z * (1 / 2 - BER) + (2 - BER));
				double LowerBound = BER - 1 / sqrt(receivedBits_mid) * z  * sqrt(BER*(1 - BER)) + 1 / (3 * receivedBits_mid)*(2 * z * z * (1 / 2 - BER) - (1 + BER));
				...
				myfile << "Upper Bound = " << UpperBound << "\n";
				myfile << "Lower Bound = " << LowerBound << "\n";
				myfile << "Number of errors = " << receivedBits_mid - coincidences_mid << "\n";
				myfile << "Number of received bits = " << receivedBits_mid << "\n";
				myfile.close();
