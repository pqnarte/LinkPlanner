...
//const long int MAX_Sink_LENGTH = 100000;		// Maximum Sink Block number of values	(to be delete 2018/07/15)
...
using t_photon = struct { t_real probabilityAmplitude;  t_real polarization; };
using t_photon_mp = struct { t_photon path[MAX_NUMBER_OF_PATHS]; };
using t_photon_mp_xy = struct { t_complex_xy path[MAX_NUMBER_OF_PATHS]; };
...
enum class signal_value_type {BinaryValue, IntegerValue, RealValue, ComplexValue, ComplexValueXY, PhotonValue, PhotonValueMP, PhotonValueMPXY, Message};
enum class signal_type { Binary, TimeDiscreteAmplitudeContinuousReal };
...
// The hierarchy of the signals are the following: the root is this abstract class, where all comum data and functions is declared,
// the second level is a template where value dependent data and member functions are declared,
// the third level is where are implementd data and member functions specifiy for a given signal type
// All signals have associated a circular buffer (temporary storage, before and after processing) and a file (permanente storage)
//#########################################################################################################################################################
...
public:
	...
	// Signal destructors
	~Signal() { if (!(valueType == signal_value_type::Message)) { delete[] buffer; }; };	

	// Buffer manipulation funtions
	t_unsigned_long ready();										// Returns the number of samples in the buffer ready to be processed
	t_unsigned_long space();										// Returns the signal buffer space without overlap
...
private:
	...
	t_unsigned_long inPosition{ 0 };									// Next position for the buffer input values
	t_unsigned_long outPosition{ 0 };									// Next position for the buffer output values
	bool bufferEmpty{ true };											// Flag bufferEmpty
	bool bufferFull{ false };											// Flag bufferFull
	const t_unsigned_long bufferLength{ DEFAULT_BUFFER_LENGTH };		// Buffer length
...
public:
	...
	void bufferGet(value_type* valueAdress) { Signal::bufferGet(valueAdress); }
...
using Binary = BaseSignal< t_binary, signal_type::Binary, signal_value_type::BinaryValue > ;
using TimeDiscreteAmplitudeContinuousReal = BaseSignal<t_real, signal_type::TimeDiscreteAmplitudeContinuousReal, signal_value_type::RealValue >;
...
public:
	...
	SuperBlock(initializer_list<Signal *> inputSignal, initializer_list<Signal *> outputSignal) :Block(inputSignal, outputSignal){ setSaveInternalSignals(false); };
