...
If she remains with the average number of photons by impulse equal to the value that Alice uses she is detected mostly due to the increase in QBER and by lowering by at least one order of magnitude all the others parameters ($D_B$, $D_{M1}$ and $D_{M2}$).
...
And even if her efficiency is of 100 \%, Eve presence still increases the ratio $(D_{M1}+D_{M2})/(D_B)$ by almost one percent, as can be seen in Table \ref{IR3}, besides also increases the QBER by nearly one order of magnitude.
...
In this work was simulated a basic intercept-resend attack, I tested for different conditions in Eve's efficiency detector and in her average number of photons by impulse. 

The results allow us to conclude that the COW protocol, although being a relatively simple protocol, is robust enough against a basic IR attack even in "paranoid" properties like her detectors having a 100\% efficiency and no Dark Counts that is not very realistic while Bob's ones remain with 10 \% and with probabilities of Dark Counts.

It is also worth noting that all these simulations are tested only for 0.1 seconds of the protocol running if we increase the time the effects of Eve are even more evident and noticeable.

In general, as far as I know and also based on the results, the COW protocol is a very good choice for a quantum key distribution, that justifies the reason why it is already being commercialized.
...
{\scriptsize \lstinputlisting{./sdf/tq_76558_cow_protocol/figures_raw/simulacao.m}}

\subsection{B. Created Matlab Functions}

{\scriptsize \lstinputlisting{./sdf/tq_76558_cow_protocol/figures_raw/fun.m}}
